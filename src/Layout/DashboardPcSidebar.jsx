...
const adminNavigation = [
  ...
  { name: "Manage Users", href: "/dashboard/manageusers", icon: UsersIcon },
];
...
const DashboardPcSidebar = () => {
  ...
  if (isLoading || !user) {
    return (
      <div className="hidden lg:flex lg:fixed lg:inset-y-0 lg:w-72 bg-cream items-center justify-center">
        <div className="w-10 h-10 mx-auto flex justify-center mt-10 items-center border-4 border-dashed rounded-full animate-spin dark:border-primary-green"></div>
      </div>
    );
  }
              ...
              return (
                <li key={item.name}>
                  <Link
                    to={item.href}
                    className={cn(
                      isActive ? "bg-primary-green text-white" : "text-primary-text hover:bg-primary-green hover:text-white",
                      "group flex items-center gap-x-3 rounded-md p-2 text-sm font-semibold"
                    )}
                  >
                    <item.icon
                      className={cn(
                        isActive ? "text-white" : "text-primary-text group-hover:text-white",
                        "size-6 shrink-0"
                      )}
                    />
                    {item.name}
